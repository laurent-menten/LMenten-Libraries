...
public class ExportedRegistry
{
	...
	public static void initializeWithInheritedChannel( Remote proxy, String name, int port )
		throws IOException
	{
		synchronized( ExportedRegistry.class )
		{
			if( initialized )
			{
		...
		if( channel instanceof ServerSocketChannel ) // (x)inetd
		{
            serverSocket = ((ServerSocketChannel) channel).socket();
		}
		else if( channel == null ) // command line
		{
			if( port == 0 )
			{
		...
		{
			throw new IOException( "unexpected channel returned from inheritedChannel: " + channel.toString() );
		}
